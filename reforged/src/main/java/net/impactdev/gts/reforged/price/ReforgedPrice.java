...
public class ReforgedPrice implements SpongePrice<ReforgedPrice.PokemonPriceSpecs, StoragePosition> {
    ...
    private final PokemonPriceSpecs price;
    ...
    public TextComponent getText() {
        ...
        this.tryAppend(builder, b -> {
            ...
            if(form == null) {
                GTSPlugin.instance().logger().info(this.price.getSpecies().getName());
                GTSPlugin.instance().logger().info(this.price.getForm());
                b.append(Component.text("[Form Resolve Error]").color(NamedTextColor.RED)).append(Component.space());
                return;
            }
    ...
    private static ReforgedPrice deserialize(JsonObject json) {
        ...
        PokemonPriceSpecs specs = PokemonPriceSpecs.deserialize(price);
    ...
    public static ItemStack getPicture(Species species, Stats form) {
        ...
        rep.setForm(form);
        return (ItemStack) (Object) (SpriteItemHelper.getPhoto(
                aprilFools ? PokemonFactory.create(PixelmonSpecies.BIDOOF.getValueUnsafe()) : rep
        ));
    }

    public static class PokemonPriceSpecs implements Storable {
        ...
        public JObject serialize() {
            return new JObject()
                    .add("price", new JObject()
                            .add("version", this.getVersion())
                            .add("species", this.species.getName())
                            .add("form", this.form)
                            .add("level", this.level)
                            .add("allowEggs", this.allowEggs)
                    );
        }

        public static PokemonPriceSpecs deserialize(JsonObject json) {
            Species species = PixelmonSpecies.get(json.get("species").getAsString()).get().getValueUnsafe();
            ...
            if(json.has("version")) {
                ...
                if (version != 2) {
                    int index = json.get("form").getAsInt();
                    LegacyKey s = LegacyKey.of(index, species.getRegistryValue());
                    LegacyKey f = LegacyKey.of(index);

                    if(translations.containsKey(s)) {
                        form = translations.get(s).name();
                    } else {
                        if(translations.containsKey(f)) {
                            form = translations.get(f).name();
                        } else {
                            form = PixelmonForms.NONE;
                        }
                    }
                } else {
                    form = json.get("form").getAsString();
                }
            } else {
                // Assume version 1
                int index = json.get("form").getAsInt();
            ...
            return new PokemonPriceSpecs(
                    species,
                    form,
                    json.get("level").getAsInt(),
                    json.get("allowEggs").getAsBoolean()
            );
        }
    }
